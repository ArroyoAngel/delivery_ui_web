...
export default function MyMarketProductsPage() {
    ...
    <div className="space-y-5">
      ...
      <div className="flex items-center gap-3">
        ...
        <Button
          variant="primary"
          size="sm"
          onClick={() => router.push('/dashboard/my-market/products/new')}
        >
          <Plus size={14} />
          Agregar producto
        </Button>
      ...
      {restaurant.menuCategories?.length > 0 ? (
          ...
          <Card key={category.id} title={category.name}>
            <div className="space-y-2">
              {category.items?.length > 0 ? (
                ...
                <p className="text-sm text-gray-400 py-3 text-center">Sin productos en esta categoría</p>
              )}
            </div>
          </Card>
        ))
        ...
        <Card>
          <p className="text-center text-gray-400 py-8">No hay categorías de productos. Agrega productos para comenzar.</p>
        </Card>
      )}
    </div>
  );
