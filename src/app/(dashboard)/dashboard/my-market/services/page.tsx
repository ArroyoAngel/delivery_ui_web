...
export default function MyMarketServicesPage() {
  ...
  const [newAreaKind, setNewAreaKind] = useState('caja');
  ...
  async function handleCreateArea() {
    ...
    try {
      ...
      toast.error(getApiErrorMessage(error, 'No se pudo registrar la caja'));
    }
  }
  ...
  async function handleSubmitOrder() {
    ...
    if (serviceType === 'local') {
      ...
      if (!selected && !selectedAreaLabel.trim()) {
        toast.error('Seleccioná o escribí una caja/mostrador');
        return;
      }
    }
    ...
    <div className="space-y-5">
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        ...
        <StatCard
          ...
          value={serviceType === 'local' ? 'Venta directa' : 'Recojo'}
          icon={<Store size={20} />}
        />
        <StatCard
          label="Items en venta"
        ...
        <Card title="Venta en curso">
          <div className="space-y-3">
            ...
            {serviceType === 'local' && (
              <>
                <div>
                  <p className="text-xs font-semibold text-gray-700 mb-2">Caja / Mostrador</p>
                  <div className="grid grid-cols-2 gap-2">
                    {areas.map((area) => (
                      <button
                        key={area.id}
            ...
            <div className="rounded-xl bg-orange-50 border border-orange-100 p-3">
              ...
              <p className="text-[11px] text-gray-500 mt-1">La venta se registra directamente en estado Confirmado.</p>
            </div>
