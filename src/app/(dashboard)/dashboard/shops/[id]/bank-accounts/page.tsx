...
import { useShopBankAccounts } from '@/hooks/useFinance';
...
export default function ShopBankAccountsPage() {
  const { id } = useParams<{ id: string }>();
  const { data: accounts = [], isLoading } = useShopBankAccounts(id);

  if (isLoading) return <PageLoader />;

  return (
    <div className="space-y-5">
      <Card title="Cuentas bancarias del negocio">
