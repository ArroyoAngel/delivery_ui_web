...
import { useShopWithdrawals } from '@/hooks/useFinance';
...
export default function ShopWithdrawalsPage() {
  const { id } = useParams<{ id: string }>();
  const { data: withdrawals = [], isLoading } = useShopWithdrawals(id, 100);

  if (isLoading) return <PageLoader />;

  return (
    <div className="space-y-5">
      <Card title="Retiros del negocio">
