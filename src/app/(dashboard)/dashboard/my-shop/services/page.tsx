...
export default function ShopServicesPage() {
  ...
  const [newAreaKind, setNewAreaKind] = useState('');
  ...
  const { data: areaKindOptions = [] } = useAreaKindOptions();
  useEffect(() => {
    if (areaKindOptions.length && !newAreaKind) {
      setNewAreaKind(areaKindOptions[0].value);
    }
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [areaKindOptions]);

  const isFood = restaurant?.serviceCategory === 'food';
  const qrImage = restaurant?.qrImageUrl ?? undefined;
  const serviceModeLabel = serviceType === 'local' ? 'Consumo local' : 'Recojo';
  const submitBillPayment = 'Cobro efectivo';
  const submitQRPayment = 'Cobro en QR';
  const successMsg = 'Orden registrada y confirmada en efectivo';
  ...
  async function handleSubmitOrder() {
    ...
    if (serviceType === 'local') {
      ...
      if (!selected && !selectedAreaLabel.trim()) {
        toast.error('Seleccioná o escribí una mesa/área');
        return;
      }
    }
  ...
  if (!isFood) {
    return (
      <div className="text-sm text-gray-500 p-6 bg-white rounded-xl border border-gray-200 shadow-sm">
        Este tipo de negocio no tiene servicio en local.
      </div>
    );
  }
        ...
        <Card title="Orden en curso">
          <div className="space-y-3">
            ...
            {serviceType === 'local' && (
              <>
                <div>
                  <p className="text-xs font-semibold text-gray-700 mb-2">Mesa / Área</p>
                  <div className="grid grid-cols-2 gap-2">
                    {areas.map((area) => (
                      <button
                        key={area.id}
