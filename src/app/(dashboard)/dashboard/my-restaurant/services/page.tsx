...
import {
  useRestaurantServiceAreas,
  useCreateRestaurantServiceArea,
...
export default function RestaurantServicesPage() {
  ...
  const { data: areas = [], isLoading: loadingAreas } = useRestaurantServiceAreas();
  const createArea = useCreateRestaurantServiceArea();
  ...
  async function handleSubmitOrder() {
    ...
    if (serviceType === 'local') {
      ...
      if (!selected && !selectedAreaLabel.trim()) {
        toast.error('Selecciona o escribe una mesa/área');
        return;
      }
    }
    ...
    <div className="space-y-5">
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        ...
        <StatCard
          ...
          value={serviceType === 'local' ? 'Consumo en local' : 'Recojo en tienda'}
          icon={<Store size={20} />}
        />
        <StatCard
