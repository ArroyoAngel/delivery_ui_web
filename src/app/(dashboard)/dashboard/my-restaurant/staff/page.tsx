      ...
      {expanded && (
        <div className="border-t border-gray-100 bg-gray-50 p-4 space-y-3">
          ...
          <div className="flex justify-end gap-2 pt-1">
            <Button variant="secondary" size="sm" onClick={() => setExpanded(false)} disabled={saving}>
              Cancelar
            </Button>
...
export default function MyRestaurantStaffPage() {
  const { data: restaurant, isLoading: loadingRestaurant, isError } = useMyRestaurant();
  const restaurantId = restaurant?.id ?? '';
  const { data: staff, isLoading: loadingStaff } = useRestaurantStaff(restaurantId);
  const [showCreate, setShowCreate] = useState(false);

  if (loadingRestaurant) return <PageLoader />;
